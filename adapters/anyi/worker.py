...
class Api:
    ...
    def call(self,path,body=None,lease=None,binary=False,method=None,headers=None,timeout=90):
    ...
    def upload(self,job,name,path):
        ...
        return self.call(f'/internal/live2d/jobs/{job["id"]}/artifacts?name='+urllib.parse.quote(name,safe=''),
                         payload,job['leaseToken'],headers={'Content-Type':'multipart/form-data; boundary='+boundary,
                                                          'X-Content-SHA256':digest(path)},timeout=300)
...
def collect_delivery(workspace,destination):
    ...
    if build['status'] not in {'complete','needs_review'} or not core['passed'] or not poses['passed'] or not material['psd_roundtrip_passed']:
        raise ValueError('Generation verification did not pass')
    from scripts.visual_repair import normalize_review
    stages=build.get('stages') or {}
    review=stages.get('visual_review')
    if review is not None: review=normalize_review(review)
    visual_passed=bool(review and review['passed'])
    if (build['status']=='complete') != visual_passed:
        raise ValueError('Build status must match the exported model visual review')
    model=motion/'model'
    ...
    validation={'schemaVersion':2,'visualPassed':visual_passed,'corePassed':True,'motionPassed':True,'psdPassed':True,'refinementRequired':True,
                ...
                'visualReview':review,'reviewSummary':stages.get('review_summary','')[:200],
                'repair':stages.get('visual_repair')}
    ...
    with zipfile.ZipFile(destination/'project.zip','w',zipfile.ZIP_DEFLATED,compresslevel=4) as archive:
        ...
        for name in ['REVIEW.md','visual-repair.json']:
            if (workspace/name).is_file(): archive.write(workspace/name,name)
        for path in sorted((workspace/'visual-evidence').glob('*')):
            if path.is_file() and path.suffix in {'.json','.png'} and not path.is_symlink():
                archive.write(path,str(path.relative_to(workspace)))
