...
构建流程内置两层恢复，详见 `docs/live2d-generation.md` 的"规则恢复与监督"一节：

- 规则层（不调用模型，始终生效）：提示词生成默认请求透明背景（`IMAGE_BACKGROUND`）；进拆层前把透明或近白背景填成中性灰；拆层后按逐层深度图和前景遮罩裁掉并入图层的背景；动作检查只因脚底位移或翻转三角形未通过时按 1.0 → 0.66 → 0.33 缩小幅度重绑定；Astra 流中断重试。
- 监督层（`LIVE2D_SUPERVISOR_MODE`，默认 `shadow`）：在规划、拆层、表情三个关卡各审查一次缩略图，收尾给初版打分，失败时输出白名单内的诊断码、建议和一句给用户的中文；`act` 模式下还会执行模型选择的免费动作（修正脸眼嘴矩形、重做表情、下次重试重新规划）。每单预算：模型调用 8 次、重规划 2 次、重绑定 2 次、重做表情 1 次。付费动作（重新生成图片）只会变成建议，由用户在 App 确认。

诊断与审查记录写在任务目录的 `supervisor/`（700 权限），只上传诊断码、建议和 ≤ 200 字摘要。
